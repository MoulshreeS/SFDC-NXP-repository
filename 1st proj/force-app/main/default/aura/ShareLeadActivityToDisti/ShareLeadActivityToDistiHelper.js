({
   ...
   onLoad: function(component, event) {
        ...
        onLoad1: function(component, event) {
      
        //alert ('vjhj') ;
       var action1 = component.get('c.fetchCollateralActivity');
        // alert ('kkk') ;
         var compleadId = component.get("v.recordId");
         //alert (compleadId) ;
        action1.setParams({leadId : compleadId});
        action1.setCallback(this, function(response) {
           ...
           // alert (state) ;
            if (state === "SUCCESS") {
                //set response value in ListOfContact attribute on component.
                component.set('v.ListOfLeads1', response.getReturnValue());
                // set deafult count and select all checkbox value to false on load 
                component.set("v.selectedCount1", 0);
                component.find("box4").set("v.value", false);
                 if(response.getReturnValue().length == 1){
                    component.set("v.select2" , 1);
                }
            }
        });
    ...
    leadData : function(component, event, helper) {
      ...
      var action = component.get("c.getleads");
        ...
        var caollateralCompleadId = component.get("v.recordId");
        action.setParams({leadId : caollateralCompleadId});
    ...
    handleSendToDisti : function(component, event, Ids,Ids1) {
        //alert(handleSendToDisti);
       ...
       // var add = component.get("v.Address");
        ...
        action.setParams({leadid : compleadId,Name : nam,
                          ...
                          idss : Ids,collidss : Ids1,strt : street,
                          //ids1: collidss, strt : street,
                          Cty : City,Stat : State,
                          Cntry : Country,Zp : Zip,
                          contactId : primaryDistributorContactId,addnlcontact : AdditionalContact,
                          ownremail : owneremail,notes : notestodisti });
 ...
 //Collateral Activities Part Start
    
    leadData : function(component, event, helper) {

      var action = component.get("c.getleadsCollateral");
 		var caollateralCompleadId = component.get("v.recordId");
        action.setParams({leadId : caollateralCompleadId});
    ...
    handleSendToDisti : function(component, event, Ids , Ids1) {
       ...
       // var add = component.get("v.Address");
        ...
        var action = component.get("c.shareLead");
 		var caollateralCompleadId = component.get("v.recordId");
        action.setParams({leadid : caollateralCompleadId,Name : nam,
                          company : comp,jobdesc : job,
                          tit : title,Industry : Ind,
                          Emaill : emai,Phnn : phn,
                          opemail : optemail,dncall : dntcall,
...
//Collateral Activities Part End
