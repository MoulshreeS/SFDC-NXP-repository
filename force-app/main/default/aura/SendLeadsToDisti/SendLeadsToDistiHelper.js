({
	handleSendToDisti : function(component, event, helper) {
        ...
        var primaryAdditionalContactCorrect = '';
         var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
            if(primaryAdditionalContact != null){
        
      
        var myarray = primaryAdditionalContact.split(',');
                //alert('szdvcs' + myarray);
                  for(var i = 0; i < myarray.length; i++)
                      //alert(myarray);
        {
        if (reg.test(myarray[i].trim()) == true) 
            {

                primaryAdditionalContactCorrect =  primaryAdditionalContactCorrect + myarray[i].trim() + ',';
              
                 
            }
        }
// For removing the Comma at the End.
                var PriamryAdditionalCorrect = primaryAdditionalContactCorrect.slice(0, -1);
                 //alert('pac' + PriamryAdditionalCorrect);
            }
        ...
        //start
         var SecondaryAdditionalContactCorrect = '';
         var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
            if(secondaryAdditionalContact != null){
      
        var myarray = secondaryAdditionalContact.split(',');
                //alert('1' + myarray);
                  for(var i = 0; i < myarray.length; i++)
        {
        if (reg.test(myarray[i].trim()) == true)
            {
                ...
                SecondaryAdditionalContactCorrect =  SecondaryAdditionalContactCorrect + myarray[i].trim() + ',' ;
               
                 
            }
        }
// For removing the Comma at the End.
//alert('2' + SecondaryAdditionalContactCorrect);
                var SecondaryAdditionalCorrect = SecondaryAdditionalContactCorrect.slice(0, -1);
                 //alert('3' + SecondaryAdditionalCorrect);
            }
